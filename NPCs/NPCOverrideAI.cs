...
{
    public static class NPCOverrideAI
    {
		...
		public static void AI_007_TownEntities(NPC npc)
		{
			...
			if (npc.townNPC || npc.type == NPCID.SkeletonMerchant)       //按流程强化NPC防御和伤害
			{
				...
				if (SomeUtils.BuffNPC())             //战斗状态，没装武器的加成
				{
					int k = 1;
			...
			else if (npc.ai[0] == TalkingPartyToPlayer || npc.ai[0] == NPCOverrideAI.TalkingToPlayer || npc.ai[0] == TalkingBartenderToPlayer)
			{
			...
			else if (npc.ai[0] == ThrowerAtk)               //投手攻击
			{
				...
				npc.ai[1] -= 1f;
				npc.localAI[3] += 1f;
				if (npc.localAI[3] == AtkDelay && Main.netMode != NetmodeID.MultiplayerClient && Target != -1)
				{
					Vector2 vector = -Vector2.UnitY;

					float dist = Main.npc[Target].Distance(npc.Center);
					if (dist > AtkRange) AtkRange = dist;
					vector = npc.DirectionTo(Main.npc[Target].Center + new Vector2(0f, -gravityCorrection * MathHelper.Clamp(npc.Distance(Main.npc[Target].Center) / AtkRange, 0f, 1f)));
					...
					if (!ArmedGNPC.GetWeapon(npc).IsAir && npc.type != NPCID.Nurse)
					{
						...
						{
							Main.PlaySound(ArmedGNPC.GetWeapon(npc).UseSound, npc.Center);
						}
					}
				}
				if (npc.ai[1] <= 0f)
						...
						foreach (NPC target in Main.npc)
						{
							if (target.active && target.immune[npc.whoAmI] == 0 && !target.dontTakeDamage && !target.friendly && rectangle3.Intersects(target.Hitbox) && (target.noTileCollide || Collision.CanHit(npc.position, npc.width, npc.height, target.position, target.width, target.height)))
							{
								...
								if (!ArmedGNPC.GetWeapon(npc).IsAir)
								{
									...
									{
										ArmedGNPC.GetWeapon(npc).modItem.OnHitNPC(Main.LocalPlayer, target, AtkDmg, AtkKB, crit);
									}
									else
									{
										switch (ArmedGNPC.GetWeapon(npc).type)
										{
											case ItemID.TheHorsemansBlade:
												SomeUtils.PumpkinSword(target.whoAmI, AtkDmg, AtkKB);
												break;
											case ItemID.BeeKeeper:
												SomeUtils.BeeKeeperHit(target, rectangle3, AtkDmg);
												break;
											case ItemID.Bladetongue:
												...
												Vector2 vector38 = new Vector2(rectangle3.X + Main.rand.Next(rectangle3.Width), rectangle3.Y + Main.rand.Next(rectangle3.Height));
												vector38 = (vector38 + npc.Center * 2f) / 3f;
												int protmp = Projectile.NewProjectile(vector38, vector37, ProjectileID.IchorSplash, (int)(AtkDmg * 0.7), AtkKB * 0.7f, Main.myPlayer, 0f, 0f);
												Main.projectile[protmp].npcProj = true;
												Main.projectile[protmp].usesLocalNPCImmunity = true;
												if (Main.projectile[protmp].localNPCHitCooldown > 10)
												{
			...
			if (Main.netMode != NetmodeID.MultiplayerClient && (npc.townNPC || npc.type == NPCID.SkeletonMerchant) && !TalkingToPlayer)
			{
				...
				else if (NotAtk && npc.ai[0] == Default && npc.velocity.Y == 0f && Main.rand.Next(1200) == 0)
				{
					...
					foreach (Player player in Main.player)
					{
						if (player.active && !player.dead && player.Distance(npc.Center) < 150 && Collision.CanHitLine(npc.Top, 0, 0, player.Top, 0, 0))
						{
							...
							npc.ai[0] = NPCOverrideAI.TalkingToPlayer;
							npc.ai[1] = TalkTime;
				...
				if (CanAtk && NPCID.Sets.AttackType[npc.type] == 0 && (SomeUtils.AttackMode() || (npc.velocity.Y == 0f && NPCID.Sets.AttackAverageChance[npc.type] > 0 && Main.rand.Next(NPCID.Sets.AttackAverageChance[npc.type] * 2) == 0)))
				{
					...
					if (SomeUtils.BuffNPC() && !ArmedGNPC.GetWeapon(npc).IsAir)
					{
						AttackTime = ArmedGNPC.GetWeapon(npc).useAnimation;
					}
